...
class LeRobotUr5eDataConfig(DataConfigFactory):
    """
    Data config for UR5e + Robotiq 2F-85 pick-place datasets collected in MuJoCo
    (see examples/ur5e for the collection repo and the HDF5 -> LeRobot converter;
    dataset keys: image, side_image, wrist_image, state, actions).
    """

    # Our actions are absolute: [6 joint position targets, gripper command in [0, 1]].
    # Pi0 models are trained on delta actions, so convert the joint dims to deltas
    # relative to the current state; the gripper dim stays absolute.
    use_delta_joint_actions: bool = True
    # If provided, will be injected into the input data if the "prompt" key is not present.
    default_prompt: str | None = None
    ...
    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        # Repack: dataset keys (right) -> inference env keys (left). Training only.
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
                    {
...
_CONFIGS = [
    ...
    #
    # UR5e + Robotiq 2F-85 pick-place in MuJoCo (see examples/ur5e).
    #
    TrainConfig(
        name="pi05_ur5e",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=10, discrete_state_input=False),
    ...
    TrainConfig(
        # LoRA variant: fits a single 24 GB GPU; usually enough for a small custom dataset.
        name="pi05_ur5e_lora",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
